...
**索引：**
- 主键索引：`PRIMARY KEY (book_id)`
- 唯一索引：`UNIQUE KEY uk_isbn (isbn)`
- 外键索引：`FOREIGN KEY (category_id) REFERENCES categories(category_id)`
- 普通索引：`INDEX idx_title (title)`
- 普通索引：`INDEX idx_author (author)`
...
**索引：**
- 主键索引：`PRIMARY KEY (cart_item_id)`
- 外键索引：`FOREIGN KEY (user_id) REFERENCES users(user_id)`
- 外键索引：`FOREIGN KEY (book_id) REFERENCES books(book_id)`
- 唯一索引：`UNIQUE KEY uk_user_book (user_id, book_id)`
...
**索引：**
- 主键索引：`PRIMARY KEY (order_item_id)`
- 外键索引：`FOREIGN KEY (order_id) REFERENCES orders(order_id)`
- 外键索引：`FOREIGN KEY (book_id) REFERENCES books(book_id)`
- 普通索引：`INDEX idx_order_id (order_id)`
